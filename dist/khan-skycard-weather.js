// khan-skycard-weather.js – Sky Edition v2.5.0
...
class KhanSkyCardEditor extends HTMLElement {
  ...
  _render() {
    ...
    shell.appendChild(makeSection('general', '⚙️', 'General', [
      ...
      divider(),
      picker('weather_entity', 'Weather Entity (sky + forecast)', true),
      switchRow('_show_weather_panel', '🌤️ Show weather panel', 'Shows current temperature and daily forecast over the upper sky area'),
      numberField('_weather_forecast_days', 'Forecast days', 1, 5, 1, 'days'),
      textField('_weather_panel_title', 'Weather panel title', 'OTTHON'),
    ]));
...
customElements.define('khan-skycard-weather-editor', KhanSkyCardEditor);
...
class KhanSkyCard extends HTMLElement {
  constructor() {
    ...
    this._weatherForecastEntity = null;
    this._weatherForecastPending = false;
    this._weatherForecastLastFetch = 0;
  ...
  static getStubConfig() {
    return {
      ...
      _show_weather_panel: true,
      _weather_forecast_days: 5,
      _weather_panel_title: 'OTTHON',
      _weather_language: 'hu',
      inverter_name: '',
  ...
  static getConfigElement() { return document.createElement('khan-skycard-weather-editor'); }
  ...
  setConfig(config) {
    ...
    const STRUCTURAL_KEYS = [
      ...
      '_show_extra_tiles','battery_cap_unit','_show_inv_banner','_show_weather_panel',
      '_weather_forecast_days','_weather_panel_title','_weather_language',
  ...
  _weatherStateObj() {
    ...
    for (const eid of candidates) {
      ...
      if (st && st.state !== 'unknown' && st.state !== 'unavailable') return { eid, st };
    }
    return { eid: null, st: null };
  }

  _weatherConditionLabel(condition) {
    const c = String(condition || '').toLowerCase().replace(/_/g, '-');
    const hu = {
      'clear-night':'Tiszta éjszaka','cloudy':'Felhős','fog':'Ködös','hail':'Jégeső',
      'lightning':'Zivatar','lightning-rainy':'Zivatar, eső','partlycloudy':'Részben felhős',
      'pouring':'Heves eső','rainy':'Esős','snowy':'Havas','snowy-rainy':'Havas eső',
      'sunny':'Napos','windy':'Szeles','windy-variant':'Szeles','exceptional':'Rendkívüli'
    };
    const en = {
      'clear-night':'Clear night','cloudy':'Cloudy','fog':'Fog','hail':'Hail','lightning':'Lightning',
      'lightning-rainy':'Thunderstorms','partlycloudy':'Partly cloudy','pouring':'Pouring rain',
      'rainy':'Rainy','snowy':'Snowy','snowy-rainy':'Snow/rain','sunny':'Sunny','windy':'Windy',
      'windy-variant':'Windy','exceptional':'Exceptional'
    };
    const lang = String(this.config._weather_language || 'hu').toLowerCase();
    return (lang.startsWith('hu') ? hu[c] : en[c]) || c || '--';
  }
  ...
  _weatherIcon(condition) {
    ...
    const map = {
      ...
      'lightning-rainy':'⛈️','partlycloudy':'🌤️','pouring':'🌧️','rainy':'🌧️',
      'snowy':'❄️','snowy-rainy':'🌨️','sunny':'☀️','windy':'💨','windy-variant':'🌬️',
      'exceptional':'⚠️'
    };
    return map[c] || '🌤️';
  }

  _weatherDayLabel(datetime) {
    if (!datetime) return '--';
    ...
    return (lang.startsWith('hu') ? daysHu : daysEn)[d.getDay()];
  }

  _weatherNum(v, digits = 0) {
    const n = Number(v);
    return Number.isFinite(n) ? n.toFixed(digits) : null;
  }

  _extractForecastResponse(result, eid) {
    const candidates = [
  ...
  async _ensureWeatherForecast(force = false) {
    if (!this.config?._show_weather_panel || !this._hass) return;
    const { eid, st } = this._weatherStateObj();
    if (!eid || !st) return;

    // Older weather integrations still expose forecast as an attribute.
    const attrForecast = st.attributes?.forecast;
    if (Array.isArray(attrForecast) && attrForecast.length) {
      this._weatherForecast = attrForecast;
...
try {
  ...
  _updateWeatherPanel() {
    ...
    if (!root) return;
    const panel = root.getElementById('kfcWeatherPanel');
    if (!panel) return;
    const show = this.config?._show_weather_panel !== false;
    panel.style.display = show ? '' : 'none';
    if (!show || !this._hass) return;

    const { st } = this._weatherStateObj();
    if (!st) return;
    const a = st.attributes || {};
    const setText = (id, value) => { const el = root.getElementById(id); if (el) el.textContent = value; };

    const title = this.config._weather_panel_title || a.friendly_name || 'OTTHON';
    setText('kfcWeatherTitle', String(title).toUpperCase());

    const temp = this._weatherNum(a.temperature, 0);
    setText('kfcWeatherTemp', temp !== null ? `${temp}°` : '--°');
    setText('kfcWeatherCondition', this._weatherConditionLabel(st.state));

    const humidity = this._weatherNum(a.humidity, 0);
    setText('kfcWeatherHumidity', humidity !== null ? `💧 ${humidity}%` : '💧 --%');

    const wind = this._weatherNum(a.wind_speed, 0);
    const windUnit = a.wind_speed_unit || 'km/h';
    setText('kfcWeatherWind', wind !== null ? `💨 ${wind} ${windUnit}` : '💨 --');

    const pressure = this._weatherNum(a.pressure, 0);
    const pressureUnit = a.pressure_unit || 'hPa';
    setText('kfcWeatherPressure', pressure !== null ? `◉ ${pressure} ${pressureUnit}` : '◉ --');

    const forecast = Array.isArray(this._weatherForecast) ? this._weatherForecast : [];
    const first = forecast[0];
    if (first) {
      const hi = this._weatherNum(first.temperature, 0);
      const lo = this._weatherNum(first.templow ?? first.temperature_low, 0);
      setText('kfcWeatherMinMax', `${hi !== null ? hi + '°' : '--°'} | ${lo !== null ? lo + '°' : '--°'}`);
    } else {
      setText('kfcWeatherMinMax', '--° | --°');
    }
    ...
    if (forecastEl) {
      ...
      forecastEl.innerHTML = items.length ? items.map(item => {
        ...
        const lo = this._weatherNum(item.templow ?? item.temperature_low, 0);
        return `<div class="kfc-wx-day">
          <div class="kfc-wx-dayname">${this._weatherDayLabel(item.datetime)}</div>
          <div class="kfc-wx-icon">${this._weatherIcon(item.condition)}</div>
          <div class="kfc-wx-high">${hi !== null ? hi + '°' : '--°'}</div>
          <div class="kfc-wx-low">${lo !== null ? lo + '°' : '--°'}</div>
        </div>`;
      }).join('') : '<div class="kfc-wx-loading">Előrejelzés betöltése…</div>';
    }
  }
  ...
  _buildStaticSVG() {
    ...
    const iconPath = '/local/community/khan-skycard-weather';    // icons served from HACS community folder
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .kfc-shell{container-type:inline-size}
      .kfc-wx-forecast{position:absolute;top:clamp(170px,20cqw,210px);left:50%;transform:translateX(-50%);width:min(66cqw,620px);display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:clamp(5px,1cqw,10px);margin:0;padding:0;border:none;z-index:6;pointer-events:none}
      .kfc-wx-day{text-align:center;min-width:0;min-height:clamp(105px,14cqw,145px);box-sizing:border-box;padding:clamp(7px,1.2cqw,12px) clamp(3px,.5cqw,5px) clamp(6px,.9cqw,9px);background:rgba(15,60,110,.18);border:1px solid rgba(255,255,255,.24);border-radius:12px;backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px);text-shadow:0 2px 4px rgba(0,0,0,.80)}
      .kfc-wx-day:last-child{border-right:none}
      .kfc-wx-dayname{font-size:clamp(.52rem,1.15cqw,.78rem);font-weight:750;letter-spacing:.3px}
      .kfc-wx-icon{font-size:clamp(1.25rem,3.2cqw,2.2rem);line-height:1.2;margin:clamp(4px,.9cqw,9px) 0 clamp(3px,.6cqw,6px)}
      .kfc-wx-high{font-size:clamp(.95rem,2.1cqw,1.45rem);font-weight:750;display:inline-block;margin-right:clamp(2px,.5cqw,6px)}
      .kfc-wx-low{font-size:clamp(.60rem,1.3cqw,.9rem);opacity:.85;display:inline-block}
      .kfc-wx-loading{grid-column:1/-1;font-size:.60rem;opacity:.65;text-align:center;padding:5px}
      @container (max-width:700px){.kfc-wx-forecast{width:74cqw;top:clamp(165px,22cqw,190px);gap:5px}.kfc-wx-day{min-height:115px;padding:7px 2px 6px;border-radius:9px}.kfc-wx-dayname{font-size:.55rem}.kfc-wx-icon{font-size:1.25rem;margin:5px 0 3px}.kfc-wx-high{font-size:.92rem;margin-right:2px}.kfc-wx-low{font-size:.58rem}}
      @container (max-width:480px){.kfc-weather-panel{top:8px;left:10px}.kfc-wx-title{font-size:.48rem;margin-bottom:1px}.kfc-wx-current{gap:5px;min-height:32px}.kfc-wx-temp{font-size:2.1rem;letter-spacing:-1px}.kfc-wx-condition{font-size:.55rem}.kfc-wx-minmax{font-size:.43rem;margin-top:1px}.kfc-wx-details{gap:5px;font-size:.42rem;margin-top:3px}.kfc-wx-forecast{width:94cqw;top:145px;gap:3px}.kfc-wx-day{min-height:78px;height:78px;padding:4px 1px 3px;border-radius:6px;background:rgba(15,60,110,.10)}.kfc-wx-dayname{font-size:.43rem}.kfc-wx-icon{font-size:.88rem;margin:3px 0 2px}.kfc-wx-high{font-size:.68rem;margin-right:1px}.kfc-wx-low{font-size:.45rem}.kfc-wx-loading{font-size:.48rem;padding:2px}}
    </style>
    <div class="kfc-shell" id="kfcShell">
      <div id="kfcSkyDiv" aria-hidden="true"></div>
      <div id="kfcWeatherPanel" class="kfc-weather-panel" style="display:${this.config._show_weather_panel === false ? 'none' : ''}">
        <div id="kfcWeatherTitle" class="kfc-wx-title">${(this.config._weather_panel_title || 'OTTHON').toUpperCase()}</div>
        <div class="kfc-wx-current">
          <div id="kfcWeatherTemp" class="kfc-wx-temp">--°</div>
          <div class="kfc-wx-meta">
            <div id="kfcWeatherCondition" class="kfc-wx-condition">--</div>
            <div id="kfcWeatherMinMax" class="kfc-wx-minmax">--° | --°</div>
          </div>
        </div>
        <div class="kfc-wx-details">
          <span id="kfcWeatherHumidity">💧 --%</span>
  ...
  // Images live at: /local/community/khan-skycard-weather/sky/
  ...
  _renderSky(sun, condition = 'clear') {
    ...
    const BASE  = '/local/community/khan-skycard-weather/sky';
...
window.customCards.push({
  type: 'khan-skycard-weather',
  name: 'Khan SkyCard Weather',
  ...
  preview: true,
  version: '2.6.0-weather',
});
customElements.define('khan-skycard-weather', KhanSkyCard);
